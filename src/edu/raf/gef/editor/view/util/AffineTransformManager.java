...
public class AffineTransformManager extends TransientObservable {
	...
	/**
	 * The transform should be made so that the userSpaceLocation matches the
	 * deviceSpaceLocation
	 */
	...
	public synchronized void bestFit() {
		...
		Point2D newDeviceSpace = new Point2D.Double(canvasDimension.getWidth() / 2, canvasDimension
				.getHeight() / 2);
		double newScale = Math.min((canvasDimension.getWidth() - border * 2) / (maxx - minx),
	...
	public synchronized void matchTransform() {
		...
		transform.translate(deviceSpaceLocation.getX() / scaleValue, deviceSpaceLocation.getY()
				/ scaleValue);
		try {
	...
	public synchronized void setScale(double scale) {
		this.scaleValue = Math.exp(Math.floor(Math.log(scaleValue) / SCALE_ROUNDING_FACTOR)
				* SCALE_ROUNDING_FACTOR);
